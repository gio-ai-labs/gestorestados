...
@Component({
  ...
  template: `
    <div class="grid place-items-center px-6 py-16 text-center">
      <div class="mb-4 grid size-12 place-items-center rounded-2xl bg-slate-100 text-slate-400 dark:bg-dark-sidebar dark:text-slate-500">
      ...
      <h3 class="font-display text-[16px] font-bold text-ink-900 dark:text-slate-100">{{ titulo() }}</h3>
      @if (mensaje()) {
        <p class="mt-1 max-w-sm text-[14px] text-slate-500 dark:text-slate-400">{{ mensaje() }}</p>
      }
      <div class="mt-5">
