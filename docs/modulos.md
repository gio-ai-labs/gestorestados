...
| shared/ui | Librería de componentes UI | ✅ | button, card, modal, table, toast, confirm, spinner, form-field, empty-state, estado-chip |
| core | Config, models, HTTP interceptors | ✅ | api.config, models/*, error.interceptor, problem-detail.util |
...
| proyecto | CRUD de proyectos | ✅ | `/api/v1/proyectos` · raíz de la jerarquía |
| sprint | CRUD de sprints + cambio de estado + tablero compuesto | ✅ | `/api/v1/sprints` · horas rollup desde workitems · `GET /api/v1/sprints/{sprintId}/tablero` retorna workitems del sprint con sus tareas en una sola respuesta (JOIN FETCH, sin N+1) — PBI #9 |
